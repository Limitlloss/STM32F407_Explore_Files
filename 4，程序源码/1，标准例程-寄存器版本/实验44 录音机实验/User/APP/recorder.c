 ...
 * @retval      无
 */
void recoder_i2s_dma_rx_callback(void)
{
...
void recoder_enter_rec_mode(void)
{
    ...
    i2s_rx_callback = recoder_i2s_dma_rx_callback;                                      /* 回调函数指wav_i2s_dma_callback */
...
void wav_recorder(void)
{
    ...
    for (i = 0; i < REC_I2S_RX_FIFO_SIZE; i++)
    {
        myfree(SRAMIN, p_i2s_recfifo_buf[i]);   /* SAI录音FIFO内存释放 */
    }
