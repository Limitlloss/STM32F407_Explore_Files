...
uint8_t g_usart_rx_buf[USART_REC_LEN] __attribute__ ((at(0X20001000))); /* 指定数组位置 */

#else      /* 使用AC6编译器时 */

/* 接收缓冲, 最大USART_REC_LEN个字节. */
uint8_t g_usart_rx_buf[USART_REC_LEN] __attribute__((section(".bss.ARM.__at_0X20001000"))); /* 指定数组位置 */
